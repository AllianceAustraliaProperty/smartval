...
/**
 * Dependency-free rich text editor built on contentEditable. Provides a
 * formatting toolbar plus a raw-HTML source mode, and exposes imperative
 * insert helpers (used to inject template variables / logos at the cursor).
 */
export const RichTextEditor = forwardRef<RichTextEditorHandle, RichTextEditorProps>(
  ({ value, onChange, className = '' }, ref) => {
    ...
    // Keep the editable surface in sync when the value changes externally
    // (initial load, reset to default, switching back from source mode) without
    // clobbering the DOM/cursor on every keystroke.
    useEffect(() => {
    ...
    const handleImage = () => {
      ...
      if (url) {
        // Insert with sensible inline sizing so logos/signatures render in email.
